...
def load_data(file):
    ...
    try:
        ...
        return (xls.parse(sheet) for sheet in ["Issues", "Skills", "Worklogs", "Non_Availability"])
...
view = st.sidebar.radio("Go to", [
    "PM Daily Brief", "Resource Utilization", "Skill Distribution", "Sprint Burnup",
    "Work Calendar", "Gantt Chart", "Traffic Light Matrix", "Sankey Diagram",
    "Radar Chart", "GPT Insight Widgets"
])
...
if issues_df is not None:
    if view == "PM Daily Brief":
        ...
        fig = px.bar(pivot, x=pivot.index, y=pivot.columns, title="Weekly Resource Utilization (hrs)", labels={'value': 'Hours Worked', 'Week': 'Week'})
        ...
        fig = px.pie(skill_counts, names='Skillset', values='Count', title="Skill Distribution", hole=0.4)
        st.plotly_chart(fig, use_container_width=True)
        ...
        burnup_data['Completed'] = burnup_data['Date'].apply(lambda d: issues_df[(issues_df['Status'] == 'Done') & (issues_df['Due Date'] <= d)]['Story Points'].sum())
        ...
        fig.add_trace(go.Scatter(x=burnup_data['Date'], y=[burnup_data['Total Scope'].iloc[0]]*len(burnup_data), mode='lines', name='Total Scope', line=dict(dash='dash')))
        ...
        fig = px.timeline(gantt_data, x_start="Start Date", x_end="Due Date", y="Assignee", color="Project", hover_name="Summary", title="Gantt Chart by Assignee")
        fig.update_yaxes(autorange="reversed")
        st.plotly_chart(fig, use_container_width=True)
    elif view == "Traffic Light Matrix":
        today = pd.to_datetime("today").normalize()
        issues_df['Due Date'] = pd.to_datetime(issues_df['Due Date'], errors='coerce')
        summary = issues_df.groupby('Assignee').agg(total_tasks=('Issue Key', 'count'), overdue_tasks=('Due Date', lambda d: (d < today).sum())).reset_index()
        summary['Status'] = summary.apply(lambda row: '🟢' if row['overdue_tasks'] == 0 else ('🟠' if row['overdue_tasks'] < row['total_tasks'] * 0.5 else '🔴'), axis=1)
        ...
        for skill in radar_data['Skillset'].unique():
            ...
            fig = go.Figure()
            fig.add_trace(go.Scatterpolar(r=df['Time Spent (hrs)'], theta=df['Resource'], fill='toself', name=skill))
            fig.update_layout(polar=dict(radialaxis=dict(visible=True)), showlegend=True, title=f"Load Balance for Skill: {skill}")
            st.plotly_chart(fig, use_container_width=True)
